...
class CicloEscolar
{
    ...
    function __construct($id_ciclo_escolar)
    {
		...
		$this->id_colegio       = $ciclo_escolar['IDColegio'];
    }
    ...
    # Métodos estáticos
    static function getLista($idcolegio)
    {
        // id_ciclo_escolar | ciclo_escolar
        return Database::select("SELECT id_ciclo_escolar, CONCAT(CAST(YEAR(fecha_inicio) AS CHAR), ' - ',
            YEAR(fecha_fin)) AS ciclo_escolar, fecha_inicio, fecha_fin, Nombre
            FROM ciclo_escolar INNER JOIN colegio ON ciclo_escolar.IDColegio = colegio.IDColegio 
			WHERE ciclo_escolar.IDColegio = '$idcolegio' ORDER BY fecha_inicio DESC");
    }

    static function getListaProximos($idcolegio)
    {
        $query = "SELECT id_ciclo_escolar, CONCAT(CasT(YEAR(fecha_inicio) AS CHAR), ' - ', YEAR(fecha_fin)) AS ciclo
            FROM ciclo_escolar WHERE fecha_fin >= NOW() AND IDColegio = '$idcolegio' ORDER BY fecha_inicio DESC";
        return Database::select($query);
    }
    ...
    static function insert($fecha_inicioVal, $fecha_finVal, $idcolegio)
    {
    ...
    static function getActual($idcolegio)
    {
        $ciclo_actual = Database::select("SELECT * FROM ciclo_escolar WHERE NOW() BETWEEN fecha_inicio AND fecha_fin AND IDColegio = '$idcolegio' LIMIT 1");
