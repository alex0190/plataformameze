...
class Alumno extends Persona
{
    ...
    function __construct($id_persona,$id_colegio)
    {
        $ciclo_actual = CicloEscolar::getActual($id_colegio)->id_ciclo_escolar;
        if(is_null($ciclo_actual)) $ciclo_actual = 0;
        $query = "SELECT persona.*, grado.id_grado, grado, grupo.id_grupo, grupo,
            COALESCE(beca, 0) AS beca, Nombre FROM persona
			...
			ON beca.id_alumno = persona.id_persona LEFT JOIN colegio ON persona.IDColegio = colegio.IDColegio
		...
		$this->idcolegio            = $persona['IDColegio'];
    }
    ...
    {
        $id_ciclo = CicloEscolar::getActual($this->idcolegio)->id_ciclo_escolar;
        $query = "SELECT materia.id_materia, id_clase, materia
            FROM alumno_grupo
            JOIN clase ON clase.id_grupo = alumno_grupo.id_grupo
            JOIN grupo ON alumno_grupo.id_grupo = grupo.id_grupo
    ...
    {
        $ciclo_actual = CicloEscolar::getActual($this->idcolegio);
        $query = "SELECT beca.*, CONCAT(tipo_beca, '-', subtipo_beca) AS tipo FROM beca
            JOIN beca_subtipo ON beca_subtipo.id_subtipo_beca = beca.id_subtipo
            JOIN beca_tipo ON beca_tipo.id_tipo_beca = beca_subtipo.id_tipo_beca
    ...
    function getEstado()
    {
        ...
        $ciclo = CicloEscolar::getActual($this->idcolegio);
    ...
    function cambiarGrupo($id_grupo)
    {
        ...
        if($gradoA == $gradoB) // Si el grado es igual, transferir calificaciones al nuevo grupo
        {
        ...
        Database::update($query);
        return $this->id_grupo = $grupoB->id_grupo;
    }
    ...
    # Métodos estáticos
    static function getLista($idcol)
    {
        return Database::select("SELECT persona.*, grado.grado, grupo.grupo, area, colonia.nombre as Colonia FROM persona
            ...
            WHERE tipo_persona = 1 AND persona.IDColegio = '$idcol'");
    }

    public static function insert($apellido_paterno, $apellido_materno, $nombres, $sexo, $id_ciclo_escolar, $idcolegio)
    {
        if(!self::existe($nombres, $apellido_paterno, $apellido_materno))
        {
            ...
            $query = "INSERT INTO persona VALUES
                ...
                1, '$password', NOW(), null, 'photo_NA.jpg', '$sexo', '$idcolegio')";
            return Database::insert($query);
